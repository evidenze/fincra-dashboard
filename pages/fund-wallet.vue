  ...
  <div>
    <div class="pt-4 d-flex pl-3">
      <div class="ml-auto mr-auto align-self-center">
        <p class="font-weight-bold pt-3">Fund your wallet</p>
      </div>
      <div>
                ...
                <div class="form-group row mb-0 mt-3">
                  <div class="col-md-12">
                    <button id="fund-button" data-style="slide-right" type="submit"
                      class="ladda-button btn sign_btn btn-block mb-3">Fund Wallet</button>
                  </div>
                </div>
              </form>
            </div>
...
export default {
  ...
  methods: {
    async creditWallet() {
      ...
      l.start();
      try {
        let response = await this.$axios.$post(`${this.$config.apiURL}/credit`, {
          amount: this.amount,
        });
